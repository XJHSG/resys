...
public partial class Upload : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string basePath = Server.MapPath("~/Admin/Upload/" + _targDir);
        string FileNames = "";
        ...
        if (files != null)
        {
            ...
            for (int i = 0; i < files.Count; i++)
            {
                ...
                //文件上传保存物理路径
                string filePath = "Admin" + "/" + "Upload" + "/" + _targDir + "/" + fileName;
                //文件类型  FileType
                ...
                using (SqlConnection conn = new DB().GetConnection())
                {
                    StringBuilder sb = new StringBuilder("Insert into [WebFiles](FileName,ShowName,FilePath,FileSizeInKB,FileType,Extension,FolderID,FolderName,CreatorID,CDT,IsDeleted,ArticleID)");
                    sb.Append(" values(@FileName,@ShowName,@FilePath,@FileSizeInKB,@FileType,@Extension,@FolderID,@FolderName,@CreatorID,@CDT,@IsDeleted,@ArticleID)");
                    ...
                    cmd.Parameters.AddWithValue("@ArticleID", 0);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
                    conn.Open();
                    ...
                    cmd.CommandText = "Update WebFolders set Counts = " + count.ToString() + " where ID = 1";
