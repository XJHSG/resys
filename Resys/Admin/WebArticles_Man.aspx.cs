...
public partial class Admin_WebArticles_Man : System.Web.UI.Page
{
    ...
    private void MyDataBind()
    {
        ...
        StringBuilder whereStr = new StringBuilder(" where IsFinished=1 and IsDeleted=0 ");
    ...
    protected void DelBtn_Click(object sender, EventArgs e)
    {
        ...
        {
            ids = ids.Substring(1);
            Response.Redirect(Server.HtmlEncode("WebArticles_Del.aspx?IDS=" + ids));
        }
    }
    ...
    protected void OrdersBtn_Click(object sender, EventArgs e)
    {
        using (SqlConnection conn = new DB().GetConnection())
        {
            SqlCommand cmd = conn.CreateCommand();
            conn.Open();
          //  string sql = "Update WebArticles set Orders = @Orders where ID = @ID";
            for (int i = 0; i <= GridView1.Rows.Count - 1; i++)
            {
                ...
                {
                    cmd.CommandText = "Update WebArticles set Orders = " + OrdersTB.Text.Trim() + " where ID = " + GridView1.DataKeys[i].Value;
                    cmd.ExecuteNonQuery();
                    cmd.Dispose();
                }
            }
    ...
    {
        Response.Redirect(Server.HtmlEncode("/Admin/WebArticles_Add.aspx"));
    }
